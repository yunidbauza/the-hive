...
| `pnpm test:hooks` · `:statusline` · `:skills` · `:done` · `:ready` · `:back` · `:title` · `:ledger` · `:agent` | Live conformance against a **real `claude`** — hooks (~3½ min), the status line, custom skills, `/done`, the boot-ready signal, the bare-`←` claim in the built app, that an unnamed session titles itself (~2½ min), the ledger MCP tools, and the headless agent runs — hooks, `--resume`, the two-wake ledger conversation, the permission fence, and the scheduler's own clock, and two task runs live at once with distinct receipts (~1 min, after a `desktop:build`) |
