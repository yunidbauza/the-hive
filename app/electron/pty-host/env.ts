 ...
 *
 * The actual construction (`TERM`/`COLORTERM`/`PWD`, the deny-list) moved to
 * `electron/shared/config-contract.ts`'s `buildSessionEnv` in story 108's fix
 * round, so the env diagnostic — which runs in main and cannot import this
 * module — builds a probe environment identically rather than by hand. `TERM`
 * and `COLORTERM` are re-exported here so nothing that already imports them
...
export function buildEnv(
  ...
  return buildSessionEnv(base, cwd, injected);
}
