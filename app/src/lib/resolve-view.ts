 ...
 */
export type ViewState = 'picker' | 'orchestrator' | 'session' | 'agent';
 ...
 *
 * 1. **Picker wins.** It is a full-stage overlay, and it deliberately does not
 *    change `activeTab` — closing it has to return the user to whatever they
 *    were looking at, which only works if the underlying tab is untouched.
 * 2. **The orchestrator is the floor.** An `activeTab` that names no entity
 ...
 *    them home.
 */
export function resolveView({ activeTab, picker, entity }: ViewInput): ViewState {
