 ...
 * them (105, 6, 27, 111) are literals, not read back off the derived lists.
...
describe('registerRemoteProxy', () => {
  it('binds every call channel to the client', () => {
    expect(callChannels.length).toBe(105);
  ...
  it('binds no handler for an event channel', () => {
    expect(eventChannels.length).toBe(27);
  ...
  it('records every binding, so the mode can be switched back', () => {
    ...
    expect(remoteProxyBindingsSize()).toBe(111);
  });
  ...
  it('unbinds every channel and stops pumping events on reset', () => {
    ...
    // 111 (105 call + 6 notify), the same literal `records every binding`
    ...
    expect(removeHandler).toHaveBeenCalledTimes(111);
    expect(removeAllListeners).toHaveBeenCalledTimes(111);
