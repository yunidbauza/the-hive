 ...
 * them (106, 6, 27, 112) are literals, not read back off the derived lists.
...
describe('registerRemoteProxy', () => {
  it('binds every call channel to the client', () => {
    expect(callChannels.length).toBe(106);
  ...
  it('records every binding, so the mode can be switched back', () => {
    ...
    expect(remoteProxyBindingsSize()).toBe(112);
  });
  ...
  it('unbinds every channel and stops pumping events on reset', () => {
    ...
    // 112 (106 call + 6 notify), the same literal `records every binding`
    ...
    expect(removeHandler).toHaveBeenCalledTimes(112);
    expect(removeAllListeners).toHaveBeenCalledTimes(112);
