 ...
 * header's `w-[calc(var(--cc-rail-w-left)-1rem)]` is only correct while the
 * rail paints with that same token and the header keeps its `px-4`. Comparing
 * the rendered geometry is the only check that survives either one being
 * restyled.
 */
