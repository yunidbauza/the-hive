...
| `self-review` | `mcp__hive__agents` lists `acr`: write `prs.json` with `waitingOn: acr`, then `ledger_ask to: acr` with the job in acr's own words, `reviewer <owner>/<repo>#<N> --self`, and `meta.intent: "self review for <owner>/<repo>#<N>"`; the wake ends there. No `acr` but the `pr-review` skill is on this machine: run `hive:pr-review --self <N>` in this run. Neither: `ledger_ask to: <reply-to>` with options `[reviewed elsewhere, skip the self review]` | the review's findings are in hand → `fix-self` when there are any, else `ready` |
...
| `ci` | `gh pr checks <N> --repo <owner>/<repo>`: exit 8 is pending, zero checks is not green until `gh api repos/<owner>/<repo>/actions/workflows` says the repo has none | green → `findings`; red → `ledger_ask to: fixer` with the failing checks (ends the wake), stay |
...
| `sync` (inside `approval` and `merge`) | the fixer pushed from a worktree of its own, so first `git -C <path> fetch origin && git -C <path> merge --ff-only origin/<headRef>` (re-read `headRef` in-call, quoted); a refused fast-forward is a `ledger_post` and a stop. Then behind base → `git -C <path> merge origin/<base>` and one push | |
| `approval` | `sync`; then `gh pr view <N> --repo <owner>/<repo> --json reviewDecision`. The wait is skipped when `~/.hive/config.json` lists this repo's project with `autoMerge: true` (HIVE-166; until it lands no project has it and every PR waits for an approval). Never the stages before it | `reviewDecision == APPROVED`, or the project auto-merges → `merge` |
